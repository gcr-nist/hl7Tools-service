...
public class HL7DBServiceTest extends TestCase {
	...
	public HL7DBServiceTest(String version, Integer expectedMessageCount,
			Integer expectedEventCount, Integer expectedCodeTableCount,
			Integer expectedDatatypeCount, Integer expectedSegmentCount) {
	...
	public static Collection<Object[]> initTestData() {
		Object[][] data = new Object[][] { 
				{ "2.7", 193, 313, 535, 86, 177 },
				{ "2.6", 212, 336, 538, 91, 174 },
				{ "2.5.1", 190, 316, 477, 91, 153 },
				{ "2.5", 191, 316, 477, 90, 153 },
				{ "2.4", 182, 291, 408, 243, 141 },
				{ "2.3.1", 134, 195, 306, 165, 113 },
				{ "2.3", 124, 185, 292, 140, 112 },
				{ "2.2", 1, 56, 189, 85, 63 }, 
				{ "2.1", 1, 37, 128, 23, 40 } };
	...
	public void testGetSupportedHL7Versions() {
		...
		if ("2.5.1".equals(version))
			assertEquals(
					new HashSet<String>(Arrays.asList("2.7", "2.6", "2.5", "2.5.1",
							"2.4", "2.3.1", "2.3", "2.2", "2.1")),
	...
	public void testGetMessageListByVersion() throws HL7DBServiceException {
		...
		assertNotNull(generatePrefix()
				+ " The message list shouldn't be empty.", messageList);
		Integer messageCount = messageList.size();
		assertEquals(generatePrefix() + " *** Message count failure *** ",
	...
	public void testGetCodeTableListByVersion() throws HL7DBServiceException {
		assertNotNull(service);
		List<CodeTable> codeTableList = service
				.getCodeTableListByVersion(version);
		assertNotNull(generatePrefix()
				+ " The code table list shouldn't be empty.", codeTableList);
		Integer codeTableCount = codeTableList.size();
		assertEquals(generatePrefix() + " *** CodeTable count failure *** ",
				expectedCodeTableCount, codeTableCount);
	...
	public void testGetDatatypeListByVersion() throws HL7DBServiceException {
		...
		assertNotNull(generatePrefix()
				+ " The datatype list shouldn't be empty.", datatypeList);
		Integer datatypeCount = datatypeList.size();
		assertEquals(generatePrefix() + " *** Datatype count failure *** ",
				expectedDatatypeCount, datatypeCount);
		// FIXME
	...
	public void testGetSegmentListByVersion() throws HL7DBServiceException {
		...
		assertNotNull(generatePrefix()
				+ " The segement list shouldn't be empty.", segmentList);
		Integer segmentCount = segmentList.size();
		assertEquals(generatePrefix() + " *** Segment count failure *** ",
	...
	public void testGetMessageByIdAndVersion() throws HL7DBServiceException {
		assertNotNull(service);
		gov.nist.healthcare.hl7tools.domain.Message message = service.getMessage(
				version, "ACK");
		assertNotNull(generatePrefix() + " The message shouldn't be null.",
				message);
	...
	private void runMessageAssertions(List<String[]> messageList) {
		for (String[] msg : messageList) {
			assertNotNull(msg);
			assertTrue(generatePrefix(msg)
					+ " The message id shouldn't be empty. " + msg,
					notEmpty(msg[0]));
			assertTrue(generatePrefix(msg)
					+ " The message description shouldn't be empty. " + msg,
					notEmpty(msg[1]));
			assertTrue(generatePrefix(msg)
					+ " The message section shouldn't be empty. " + msg,
					notEmpty(msg[2]));
	...
	private void run_v251_ADR_A19_Assertions(String[] msg) {
		assertEquals(generatePrefix(msg) + "*** Description Failure ***",
				"ADT response", msg[1]);
	}
	...
	private void run_v251_ORU_R01_Assertions(
			gov.nist.healthcare.hl7tools.domain.Message m) {
		assertNotNull(m);
		...
		assertEquals("[HL7 v251] ORU_R01 description should be '"
				+ expectedDesc + "'.", expectedDesc, m.getDescription());
		assertEquals("[HL7 v251] ORU_R01 event description should be '"
				+ expectedEventDesc + "'.", expectedEventDesc,
	...
	private void run_v251_ORU_R01_Structure_Assertions(List<Element> children) {
		...
		assertEquals(generatePrefix() + " ORU_R01 should have 4 children.", 4,
				children.size());
		Element msh = children.get(0);
		runElementAssertions(msh, ElementType.SEGEMENT, "MSH", 1, Usage.R, 1,
				"1");
		...
		runElementAssertions(patient_result, ElementType.GROUP,
				"ORU_R01.PATIENT_RESULT", 3, Usage.R, 1, "*");
		Element dsc = children.get(3);
		runElementAssertions(dsc, ElementType.SEGEMENT, "DSC", 4, Usage.O, 0,
				"1");
		run_v251_ORU_R01_PATIENT_RESULT_Structure_Assertions(patient_result
				.getChildren());
	}

	private void run_v251_ORU_R01_PATIENT_RESULT_Structure_Assertions(
			List<Element> children) {
		assertNotNull(children);
		assertEquals(generatePrefix()
				+ " ORU_R01.PATIENT_RESULT should have 2 children.", 2,
				children.size());
		Element patient = children.get(0);
		runElementAssertions(patient, ElementType.GROUP,
				"ORU_R01.PATIENT_RESULT.PATIENT", 1, Usage.O, 0, "1");
		Element order_observation = children.get(1);
		runElementAssertions(order_observation, ElementType.GROUP,
				"ORU_R01.PATIENT_RESULT.ORDER_OBSERVATION", 2, Usage.R, 1, "*");
	}

	private void runElementAssertions(Element e, ElementType type,
			String expectedName, int expectedPosition, Usage expectedUsage,
			int expectedMin, String expectedMax) {
		assertNotNull(e);
		assertEquals(generatePrefix(e) + " *** Name Assertion Failure ***",
		...
		// e.getPosition());
		assertEquals(generatePrefix(e) + " *** Usage Assertion Failure ***",
				expectedUsage, e.getUsage());
		assertEquals(generatePrefix(e) + " *** Min Assertion Failure ***",
				expectedMin, e.getMin());
		assertEquals(generatePrefix(e) + " *** Max Assertion Failure ***",
				expectedMax, e.getMax());
	}

	private void runSegmentLibraryAssertions(SegmentLibrary segmentLibrary) {
		assertNotNull(segmentLibrary);
		List<String> expectedSegList = Arrays.asList("MSH", "SFT", "DSC",
				"PID", "PD1", "NTE", "NK1", "PV1", "PV2", "ORC", "OBR", "CTD",
				"FT1", "CTI", "TQ1", "TQ2", "OBX", "SPM");
		Set<String> segmentSet = new HashSet<String>(expectedSegList);
		assertEquals(generatePrefix()
				+ "*** Segment Library number of elements failure ***",
				segmentSet, segmentLibrary.keySet());
